    ...
    for i in range(epoch_size):
        ...
        if visualize and motion_start_time[0] is not None:
            ...
            while not (j >= len(frames) and k >= len(x_out_seq)):
                ...
                cv2.imshow('source diff', cv2.resize(source_diff, (384, 384), interpolation=cv2.INTER_NEAREST))
                ...
                cv2.imshow('pred diff',
                           cv2.resize(pred_diff, (384, 384), interpolation=cv2.INTER_NEAREST))
                cv2.waitKey(0)
                j += 1
                k += 1
        if not visualize:
