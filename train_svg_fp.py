...
parser.add_argument('--log_dir', default='/misc/vlgscratch4/FergusGroup/denton/svg_logs/svg_fp/', help='base directory to save logs')
...
parser.add_argument('--model', default='dcgan | vgg', help='model type')
...
if opt.model_dir != '':
    ...
    name = 'model=%s%dx%d-rnn_size=%d-rnn_layers=%d-n_past=%d-n_future=%d-lr=%.4f-g_dim=%d-z_dim=%d-last_frame_skip=%d-beta=%.7f%s' % (opt.model, opt.image_width, opt.image_width, opt.rnn_size, opt.rnn_layers, opt.n_past, opt.n_future, opt.lr, opt.g_dim, opt.z_dim, opt.last_frame_skip, opt.beta, opt.name)
...
if opt.model_dir != '':
    ...
    posterior = lstm_models.gaussian_lstm(opt.g_dim, opt.z_dim, opt.rnn_size, opt.batch_size)
